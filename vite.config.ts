...
type FigmaSiteConfiguration = {
  ...
  robots?: { index?: boolean }; icons?: { icon?: string }
  openGraph?: { image?: string }; analytics?: { googleAnalyticsId?: string }
  customScripts?: { headStart?: string; headEnd?: string; bodyStart?: string; bodyEnd?: string }
  accessibility?: { addBypassLinks?: boolean }
}

const siteConfigPath = new URL('./.figma/make/site.json', import.meta.url).pathname
const siteConfiguration: FigmaSiteConfiguration = existsSync(siteConfigPath)
  ? JSON.parse(readFileSync(siteConfigPath, 'utf-8'))
  : { title: 'eSIM Generator', description: 'Buat kartu eSIM dengan QR code' }
