...
// ── URL helpers (v3) ───────────────────────────────────────────────────────
function buildShareUrl(lpa: string, customDomain?: string, theme?: string): string {
  const base = customDomain
    ? `https://${customDomain.replace(/^https?:\/\//, '').replace(/\/$/, '')}`
    : window.location.origin + window.location.pathname
  let url = `${base}?lpa=${lpa.replace(/ /g, '+')}`
...
function ESIMCard({ esim, logoUrl, index, total, t, customDomain, logoShareUrl, themeMode, pkgData, pkgDuration, pkgRegion }: { esim: ESim; logoUrl: string | null; index: number; total: number; t: typeof DARK; customDomain?: string; logoShareUrl?: string; themeMode?: string; pkgData?: string; pkgDuration?: string; pkgRegion?: string }) {
  ...
  const handleCopyLink = () => {
    copyText(buildShareUrl(esim.lpa, customDomain, themeMode as string))
    setLinkCopied(true)
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
        {options.map(o => (
          <button key={o} onClick={() => onChange(value === o ? '' : o)}
            style={{ padding: '6px 14px', borderRadius: '999px', border: `1.5px solid ${value === o ? t.teal : t.border}`, background: value === o ? t.tealDim : 'transparent', color: value === o ? t.teal : t.muted, fontSize: '12px', fontWeight: 600, fontFamily: 'Plus Jakarta Sans, sans-serif', cursor: 'pointer', transition: 'all 0.2s', boxShadow: value === o ? `0 0 12px ${t.teal}44,inset 0 0 8px ${t.teal}11` : 'none', transform: value === o ? 'scale(1.05)' : 'scale(1)' }}>
            {o}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        <div style={{ position: 'relative' }}>
          <button type="button" onClick={() => setRegionOpen(v => !v)}
            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: isDark ? 'rgba(0,0,0,0.3)' : '#fff', border: `1.5px solid ${region ? t.teal + '66' : t.border}`, borderRadius: '10px', padding: '10px 14px', color: region ? t.text : t.dim, fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: '13px', cursor: 'pointer', transition: 'border-color 0.2s', textAlign: 'left' }}>
          ...
          {regionOpen && (
            <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, background: isDark ? '#0e1812' : '#fff', border: `1.5px solid ${t.border}`, borderRadius: '10px', zIndex: 50, boxShadow: '0 8px 32px rgba(0,0,0,0.18)', display: 'flex', flexDirection: 'column' }}>
                ...
                {REGION_OPTIONS.filter(r => {
                  ...
                  return (
                    <button key={r} type="button" onClick={() => { setRegion(r); setRegionOpen(false); setRegionSearch('') }}
                      style={{ width: '100%', padding: '9px 14px', background: region === r ? t.tealDim : 'none', border: 'none', textAlign: 'left', color: region === r ? t.teal : t.text, fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: '13px', cursor: 'pointer', transition: 'background 0.15s', display: 'flex', alignItems: 'center', gap: '10px' }}>
...
function LPAInputView({ t, isDark, themeMode, setThemeMode, onLogout, username, isAdmin = false, onSwitchToMembers }: {
  t: typeof DARK; isDark: boolean; themeMode: ThemeMode; setThemeMode: (m: ThemeMode) => void; onLogout: () => void; username: string; isAdmin?: boolean; onSwitchToMembers?: () => void
}) {
  ...
  const [logoUrl, setLogoUrl] = useState<string | null>(() => localStorage.getItem('esim_logo') || DEFAULT_LOGO)
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 16px 24px', fontFamily: 'Plus Jakarta Sans, sans-serif', color: t.text, position: 'relative' }}>


      {/* Fixed header */}
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 20px', background: isDark ? 'rgba(10,15,13,0.9)' : 'rgba(255,255,255,0.9)', backdropFilter: 'blur(12px)', borderBottom: `1px solid ${t.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span onClick={isAdmin && onSwitchToMembers ? onSwitchToMembers : undefined} style={{ fontSize: '10px', background: isAdmin ? `${t.error}20` : t.tealDim, border: `1px solid ${isAdmin ? t.error + '44' : t.teal + '44'}`, color: isAdmin ? t.error : t.teal, padding: '2px 8px', borderRadius: '999px', fontWeight: 600, cursor: isAdmin && onSwitchToMembers ? 'pointer' : 'default' }}>{isAdmin ? '👑 Admin' : '⭐ Premium'}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <ThemeToggle mode={themeMode} setMode={m => { setThemeMode(m); localStorage.setItem('esim_theme', m) }} t={t} />
      ...
      <div style={{ width: '100%', maxWidth: '460px', position: 'relative', zIndex: 1 }}>
        {!isResult ? (
                ...
                <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      ...
                      <input value={logoShareUrl} onChange={(e) => { setLogoShareUrl(e.target.value); localStorage.setItem('esim_logo_url', e.target.value); if (e.target.value) setLogoUrl(e.target.value) }}
                        placeholder="https://i.imgur.com/logomu.png"
                        style={{ background: isDark ? 'rgba(0,0,0,0.35)' : 'rgba(248,250,252,0.8)', border: `1.5px solid ${logoShareUrl ? t.teal + '66' : t.border}`, borderRadius: '10px', padding: '10px 14px', color: t.text, fontFamily: 'JetBrains Mono, monospace', fontSize: '11.5px', outline: 'none', width: '100%', transition: 'border-color 0.2s' }}
                        onFocus={(e) => { e.currentTarget.style.borderColor = t.teal }}
                        onBlur={(e) => { e.currentTarget.style.borderColor = logoShareUrl ? t.teal + '66' : t.border }} />
          ...
          <>
            ...
            <ESIMCard key={activeIndex} esim={esims[activeIndex]} logoUrl={logoUrl} index={activeIndex} total={esims.length} t={t} themeMode={themeMode} customDomain={customDomain} logoShareUrl={logoShareUrl || undefined} pkgData={pkgData} pkgDuration={pkgDuration} pkgRegion={pkgRegion} />
            {esims.length > 1 && (
...
function AdminPanel({ t, isDark, profile, onLogout, themeMode, setThemeMode }: { t: typeof DARK; isDark: boolean; profile: Profile; onLogout: () => void; themeMode: ThemeMode; setThemeMode: (m: ThemeMode) => void }) {
  ...
  if (tab === 'generator') {
    return (
      <LPAInputView t={t} isDark={isDark} themeMode={themeMode} setThemeMode={setThemeMode} onLogout={onLogout} username={(profile as any).username || profile.email.split('@')[0]} isAdmin={true} onSwitchToMembers={() => setTab('members')} />
    )
  }
...
function MemberDashboard({ t, isDark, profile, onLogout, themeMode, setThemeMode }: { t: typeof DARK; isDark: boolean; profile: Profile; onLogout: () => void; themeMode: ThemeMode; setThemeMode: (m: ThemeMode) => void }) {
  ...
  return <LPAInputView t={t} isDark={isDark} themeMode={themeMode} setThemeMode={setThemeMode} onLogout={onLogout} username={(profile as any).username || profile.email.split('@')[0]} />
}
...
export default function App() {
  ...
  const [inputMounted, setInputMounted] = useState(false)
  const [themeMode, setThemeMode] = useState<ThemeMode>(() => getThemeFromUrl() || (localStorage.getItem('esim_theme') as ThemeMode) || 'dark')
  const fileRef = useRef<HTMLInputElement>(null)
  ...
  // Shared link → langsung tampil QR, tidak perlu login
  if (!hasLpaInUrl) {
    ...
    // Tidak login & tidak ada LPA → wajib login
      ...
      <div style={{ width: '100%', maxWidth: '460px', position: 'relative', zIndex: 1 }}>
        {!isResult ? (
          ...
          <>
            ...
            <ESIMCard key={activeIndex} esim={esims[activeIndex]} logoUrl={logoUrl} index={activeIndex} total={esims.length} t={t} customDomain={customDomain} logoShareUrl={logoShareUrl || undefined} themeMode={themeMode} />
