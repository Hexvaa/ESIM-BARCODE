  ...
  return (
    <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: '22px', overflow: 'hidden', boxShadow: t.shadow, transition: 'opacity 0.4s, transform 0.4s', opacity: mounted ? 1 : 0, transform: mounted ? 'translateY(0) scale(1)' : 'translateY(20px) scale(0.98)', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', padding: '32px 28px 20px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse 50% 70% at 80% 80%, rgba(124,58,237,0.07) 0%, transparent 60%)`, pointerEvents: 'none' }} />
        {/* subtle grid texture */}
        ...
        {[...Array(6)].map((_, i) => (
          <div key={i} style={{ position: 'absolute', width: i % 2 === 0 ? '6px' : '4px', height: i % 2 === 0 ? '6px' : '4px', borderRadius: '50%', background: i % 3 === 0 ? t.teal : i % 3 === 1 ? 'rgba(124,58,237,0.6)' : t.teal, opacity: 0.35 + (i * 0.08), left: `${10 + i * 14}%`, top: `${15 + (i % 3) * 25}%`, animation: `ambientDot${i % 3} ${3 + i * 0.7}s ease-in-out infinite`, pointerEvents: 'none' }} />
        ))}
        {total > 1 && (
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          {/* prismatic halo */}
          <div style={{ position: 'absolute', inset: '-20px', borderRadius: '28px', background: `conic-gradient(from 0deg, ${t.teal}28, rgba(124,58,237,0.15), ${t.teal}28, rgba(124,58,237,0.15), ${t.teal}28)`, animation: 'spinSlow 8s linear infinite', filter: 'blur(8px)', pointerEvents: 'none' }} />
          {/* pulsing outer glow */}
          ...
          <div style={{ position: 'absolute', inset: '-18px', borderRadius: '28px', border: `1px dashed rgba(124,58,237,0.25)`, animation: 'spinSlow 18s linear infinite reverse', pointerEvents: 'none' }} />
          {/* corner brackets */}
          ...
          <div style={{ transition: 'opacity 0.5s, transform 0.5s', opacity: inputMounted ? 1 : 0, transform: inputMounted ? 'translateY(0)' : 'translateY(28px)' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '32px', position: 'relative' }}>
              ...
              <h1 style={{ fontSize: '30px', fontWeight: 800, color: t.text, margin: '0 0 8px', letterSpacing: '-0.02em', lineHeight: 1.2, fontFamily: 'Space Grotesk, sans-serif' }}>
                <span style={{ backgroundImage: `linear-gradient(135deg,${t.teal} 0%,${isDark ? '#a78bfa' : '#7c3aed'} 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>eSIM</span>
                {' '}
                <span style={{ color: t.text }}>Roaming</span>
              </h1>
