...
function buildShareUrl(lpa: string, customDomain?: string, theme?: string, pkg?: { data?: string; duration?: string; region?: string }): string {
  const base = customDomain
    ? `https://${customDomain.replace(/^https?:\/\//, '').replace(/\/$/, '')}`
    : window.location.origin + window.location.pathname
  return `${base}?lpa=${lpa.trim().replace(/ /g, '+')}`
}
...
function getPkgFromUrl() {
  ...
  return {
    data: data ? (data.includes('GB') ? data : data + 'GB') : '',
    duration: dur ? (dur.toLowerCase().includes('hari') ? dur : dur + ' Hari') : '',
    region: region ? codeToRegion(region) : '',
  }
}
...
function ESIMCard({ esim, logoUrl, index, total, t, customDomain, logoShareUrl, themeMode, pkgData, pkgDuration, pkgRegion }: { esim: ESim; logoUrl: string | null; index: number; total: number; t: typeof DARK; customDomain?: string; logoShareUrl?: string; themeMode?: string; pkgData?: string; pkgDuration?: string; pkgRegion?: string }) {
  ...
  const handleCopyLink = () => {
    ...
    copyText(buildShareUrl(esim.lpa, customDomain, theme, { data: pkgData, duration: pkgDuration, region: pkgRegion }))
    setLinkCopied(true)
...
export default function App() {
  ...
  // Jika sudah login, selalu tampilkan dashboard (baik dari share link maupun langsung)
      ...
      <div style={{ width: '100%', maxWidth: '460px', position: 'relative', zIndex: 1 }}>
        {!isResult ? (
          ...
          <>
            ...
            <ESIMCard key={activeIndex} esim={esims[activeIndex]} logoUrl={logoUrl} index={activeIndex} total={esims.length} t={t} customDomain={customDomain} logoShareUrl={logoShareUrl || undefined} themeMode={themeMode} pkgData={urlPkg.data} pkgDuration={urlPkg.duration} pkgRegion={urlPkg.region} />
