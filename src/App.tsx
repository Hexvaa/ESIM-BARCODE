...
// ── eSIM card ──────────────────────────────────────────────────────────────
function ESIMCard({ esim, logoUrl, index, total, t }: { esim: ESim; logoUrl: string | null; index: number; total: number; t: typeof DARK }) {
  ...
  return (
    <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: '22px', overflow: 'hidden', boxShadow: t.shadow, transition: 'opacity 0.4s, transform 0.4s', opacity: mounted ? 1 : 0, transform: mounted ? 'translateY(0) scale(1)' : 'translateY(20px) scale(0.98)', position: 'relative' }}>
      ...
      {/* Download / Copy */}
      <div style={{ display: 'flex', gap: '8px', padding: '14px 24px 4px' }}>
        {[
      ...
      <div style={{ width: '100%', maxWidth: '460px', position: 'relative', zIndex: 1 }}>
        {!isResult ? (
          ...
          <>
            ...
            <ESIMCard key={activeIndex} esim={esims[activeIndex]} logoUrl={logoUrl} index={activeIndex} total={esims.length} t={t} />
