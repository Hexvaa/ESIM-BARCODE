...
function buildShareUrl(lpa: string, customDomain?: string, theme?: string, pkg?: { data?: string; duration?: string; region?: string }, userId?: string, logoShareUrl?: string): string {
  ...
  if (userId) url += `&u=${userId.slice(0, 8)}`
  if (logoShareUrl) url += `&lg=${encodeLogoForUrl(logoShareUrl)}`
  return url
}
...
function ESIMCard({ esim, logoUrl, index, total, t, customDomain, userId, logoShareUrl, themeMode, pkgData, pkgDuration, pkgRegion }: { esim: ESim; logoUrl: string | null; index: number; total: number; t: typeof DARK; customDomain?: string; userId?: string; logoShareUrl?: string; themeMode?: string; pkgData?: string; pkgDuration?: string; pkgRegion?: string }) {
  ...
  const handleCopyLink = () => {
    ...
    copyText(buildShareUrl(esim.lpa, customDomain, theme, { data: pkgData, duration: pkgDuration, region: pkgRegion }, userId, logoShareUrl))
    setLinkCopied(true)
...
function LoginPage({ t, isDark, themeMode, setThemeMode, onLogin }: { t: typeof DARK; isDark: boolean; themeMode: ThemeMode; setThemeMode: (m: ThemeMode) => void; onLogin: (p: Profile) => void }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 55; i++) {
      particles.push({ x: Math.random() * window.innerWidth, y: Math.random() * window.innerHeight, vx: (Math.random() - 0.5) * 0.4, vy: (Math.random() - 0.5) * 0.4, r: Math.random() * 1.8 + 0.4, alpha: Math.random() * 0.6 + 0.2, pulse: Math.random() * Math.PI * 2 })
    }
    let frame = 0
    const draw = () => {
      ...
      particles.forEach((p, i) => {
        ...
        ctx.fillStyle = `rgba(45,212,160,${alpha})`; ctx.fill()
        for (let j = i + 1; j < particles.length; j++) {
          const dx = particles[j].x - p.x, dy = particles[j].y - p.y
          const dist = Math.sqrt(dx * dx + dy * dy)
          if (dist < 120) {
            ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(particles[j].x, particles[j].y)
            ctx.strokeStyle = `rgba(45,212,160,${0.12 * (1 - dist / 120)})`; ctx.lineWidth = 0.5; ctx.stroke()
          }
        }
      })
  ...
  const handle = async () => {
    ...
    if (mode === 'register') {
      ...
      setRegistered(username.trim()); setLoading(false); return
    }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#060d0a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Plus Jakarta Sans, sans-serif', position: 'relative', overflow: 'hidden' }}>

      {/* Animated particle canvas */}
      <canvas ref={canvasRef} style={{ position: 'absolute', inset: 0, zIndex: 0 }} />

      {/* Deep radial glows */}
      <div style={{ position: 'absolute', inset: 0, zIndex: 0, background: 'radial-gradient(ellipse 70% 60% at 50% 50%, rgba(45,212,160,0.07) 0%, transparent 70%)' }} />
      <div style={{ position: 'absolute', top: '-20%', left: '-10%', width: '60vw', height: '60vw', borderRadius: '50%', background: 'radial-gradient(circle, rgba(45,212,160,0.05) 0%, transparent 65%)', zIndex: 0 }} />
      <div style={{ position: 'absolute', bottom: '-20%', right: '-10%', width: '50vw', height: '50vw', borderRadius: '50%', background: 'radial-gradient(circle, rgba(100,150,255,0.04) 0%, transparent 65%)', zIndex: 0 }} />

      {/* Scanning beam */}
      <div style={{ position: 'absolute', inset: 0, zIndex: 0, overflow: 'hidden', pointerEvents: 'none' }}>
        <div style={{ position: 'absolute', left: 0, right: 0, height: '1px', background: 'linear-gradient(90deg,transparent,rgba(45,212,160,0.4),transparent)', animation: 'loginScan 8s linear infinite', top: 0 }} />
      </div>
      ...
      {/* Main card */}
      <div style={{ width: '100%', maxWidth: '420px', padding: '0 20px', position: 'relative', zIndex: 1, opacity: mounted ? 1 : 0, transform: mounted ? 'translateY(0)' : 'translateY(32px)', transition: 'opacity 0.7s cubic-bezier(0.22,1,0.36,1), transform 0.7s cubic-bezier(0.22,1,0.36,1)' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <div style={{ position: 'relative', width: '100px', height: '100px', margin: '0 auto 20px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {/* Pulse rings */}
            <div style={{ position: 'absolute', inset: '-8px', borderRadius: '50%', border: '1px solid rgba(45,212,160,0.25)', animation: 'loginPulseRing 3s ease-out infinite' }} />
            <div style={{ position: 'absolute', inset: '-8px', borderRadius: '50%', border: '1px solid rgba(45,212,160,0.2)', animation: 'loginPulseRing 3s ease-out infinite 1s' }} />
            {/* Orbiting dot 1 */}
            <div style={{ position: 'absolute', inset: 0, animation: 'loginOrbit 6s linear infinite', display: 'flex', alignItems: 'flex-start', justifyContent: 'center' }}>
              <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#2dd4a0', boxShadow: '0 0 8px rgba(45,212,160,0.9)', marginTop: '-3px' }} />
            </div>
            {/* Orbiting dot 2 */}
            <div style={{ position: 'absolute', inset: '10px', animation: 'loginOrbit2 4s linear infinite', display: 'flex', alignItems: 'flex-start', justifyContent: 'center' }}>
              <div style={{ width: '4px', height: '4px', borderRadius: '50%', background: '#a78bfa', boxShadow: '0 0 6px rgba(167,139,250,0.9)', marginTop: '-2px' }} />
            </div>
            {/* Icon center */}
            <div style={{ width: '80px', height: '80px', borderRadius: '24px', background: 'linear-gradient(145deg,#0e2218,#061410)', border: '1.5px solid rgba(45,212,160,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 32px rgba(45,212,160,0.15), 0 8px 32px rgba(0,0,0,0.5), inset 0 1px 0 rgba(45,212,160,0.1)', animation: 'loginFloat 6s ease-in-out infinite', position: 'relative', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg,rgba(45,212,160,0.08) 0%,transparent 60%)' }} />
              <span style={{ fontSize: '34px', position: 'relative', zIndex: 1 }}>📶</span>
            </div>
          </div>

          <h1 style={{ fontSize: '36px', fontWeight: 800, margin: '0 0 6px', fontFamily: 'Space Grotesk, sans-serif', letterSpacing: '-0.03em', animation: 'loginGlitch 8s ease-in-out infinite' }}>
            <span style={{ backgroundImage: 'linear-gradient(135deg,#2dd4a0 0%,#6ee7c7 40%,#a78bfa 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>ToleeSim</span>
          </h1>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
            <div style={{ height: '1px', width: '28px', background: 'linear-gradient(90deg,transparent,rgba(45,212,160,0.4))' }} />
            <span style={{ fontSize: '11px', color: 'rgba(45,212,160,0.6)', fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase' }}>eSIM Roaming Generator</span>
            <div style={{ height: '1px', width: '28px', background: 'linear-gradient(90deg,rgba(45,212,160,0.4),transparent)' }} />
          </div>
        </div>

        {/* Card */}
        <div style={{ background: 'rgba(8,18,13,0.85)', backdropFilter: 'blur(28px)', border: '1px solid rgba(45,212,160,0.18)', borderRadius: '24px', padding: '32px 28px', boxShadow: '0 32px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(45,212,160,0.06), inset 0 1px 0 rgba(45,212,160,0.08)', position: 'relative', overflow: 'hidden' }}>

          {/* Top shimmer */}
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: 'linear-gradient(90deg,transparent,#2dd4a0,#a78bfa,#2dd4a0,transparent)' }} />
          {/* Corner accent */}
          <div style={{ position: 'absolute', top: 0, right: 0, width: '80px', height: '80px', background: 'radial-gradient(circle at top right,rgba(45,212,160,0.08),transparent 70%)', pointerEvents: 'none' }} />
            ...
            <div style={{ display: 'flex', gap: '4px', marginBottom: '24px', background: 'rgba(0,0,0,0.3)', borderRadius: '12px', padding: '4px' }}>
              {(['login', 'register'] as const).map(m => (
                <button key={m} onClick={() => { setMode(m); setError('') }}
                  style={{ flex: 1, padding: '8px', borderRadius: '9px', border: 'none', background: mode === m ? 'rgba(45,212,160,0.15)' : 'transparent', color: mode === m ? '#2dd4a0' : 'rgba(255,255,255,0.35)', fontSize: '12px', fontWeight: 700, fontFamily: 'Plus Jakarta Sans, sans-serif', cursor: 'pointer', transition: 'all 0.25s', letterSpacing: '0.05em', boxShadow: mode === m ? 'inset 0 0 0 1px rgba(45,212,160,0.25)' : 'none' }}>
          ...
          {registered ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '18px', textAlign: 'center', animation: 'loginFadeUp 0.5s ease-out' }}>
              <div style={{ fontSize: '48px', animation: 'loginFloat 3s ease-in-out infinite' }}>✅</div>
              <div>
                <div style={{ fontSize: '16px', fontWeight: 700, color: '#fff', marginBottom: '8px' }}>Akun <span style={{ color: '#2dd4a0' }}>@{registered}</span> berhasil dibuat!</div>
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.45)', lineHeight: 1.7 }}>Konfirmasi ke admin via WhatsApp agar akunmu segera diaktifkan.</div>
              </div>
              <a href={waUrl} target="_blank" rel="noopener noreferrer"
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', padding: '15px', background: 'linear-gradient(135deg,#25d366,#128c7e)', color: '#fff', borderRadius: '14px', textDecoration: 'none', fontSize: '14px', fontWeight: 700, boxShadow: '0 4px 24px rgba(37,211,102,0.35)', letterSpacing: '0.02em' }}>
              ...
              <button onClick={() => { setRegistered(''); setMode('login') }}
                style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', fontSize: '12px', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
                <label style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(45,212,160,0.7)', letterSpacing: '0.14em', textTransform: 'uppercase' }}>Username</label>
                <input value={username} onChange={e => setUsername(e.target.value)} placeholder="Masukkan username" autoComplete="off"
                  style={{ background: 'rgba(0,0,0,0.35)', border: '1.5px solid rgba(45,212,160,0.18)', borderRadius: '12px', padding: '13px 16px', color: '#e2e8f0', fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: '14px', outline: 'none', transition: 'all 0.25s', width: '100%' }}
                  onFocus={e => { e.currentTarget.style.borderColor = 'rgba(45,212,160,0.6)'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(45,212,160,0.08)' }}
                  onBlur={e => { e.currentTarget.style.borderColor = 'rgba(45,212,160,0.18)'; e.currentTarget.style.boxShadow = 'none' }} />
              </div>

              {/* Password */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
                <label style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(45,212,160,0.7)', letterSpacing: '0.14em', textTransform: 'uppercase' }}>Password</label>
                <div style={{ position: 'relative' }}>
                  <input value={password} onChange={e => setPassword(e.target.value)} type={showPw ? 'text' : 'password'} placeholder="Masukkan password" autoComplete="new-password"
                    style={{ background: 'rgba(0,0,0,0.35)', border: '1.5px solid rgba(45,212,160,0.18)', borderRadius: '12px', padding: '13px 46px 13px 16px', color: '#e2e8f0', fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: '14px', outline: 'none', transition: 'all 0.25s', width: '100%' }}
                    onFocus={e => { e.currentTarget.style.borderColor = 'rgba(45,212,160,0.6)'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(45,212,160,0.08)' }}
                    onBlur={e => { e.currentTarget.style.borderColor = 'rgba(45,212,160,0.18)'; e.currentTarget.style.boxShadow = 'none' }}
                    onKeyDown={e => e.key === 'Enter' && handle()} />
                  <button type="button" onClick={() => setShowPw(v => !v)}
                    style={{ position: 'absolute', right: '14px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.3)', padding: '2px', display: 'flex', alignItems: 'center' }}>
                    {showPw
              ...
              {error && (
                <div style={{ fontSize: '12px', color: '#f87171', background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.25)', borderRadius: '10px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>⚠</span> {error}
                </div>
              )}

              {mode === 'register' && (
                <div style={{ fontSize: '11px', color: 'rgba(45,212,160,0.55)', background: 'rgba(45,212,160,0.05)', border: '1px solid rgba(45,212,160,0.12)', borderRadius: '10px', padding: '10px 14px', lineHeight: 1.7 }}>
                  💡 Setelah daftar, konfirmasi ke admin via WhatsApp untuk aktivasi akun.
                </div>
              )}

              {/* Submit button */}
              <button onClick={handle} disabled={!canSubmit}
                style={{ position: 'relative', overflow: 'hidden', padding: '14px', background: canSubmit ? 'linear-gradient(135deg,#2dd4a0,#1fa876)' : 'rgba(45,212,160,0.08)', color: canSubmit ? '#fff' : 'rgba(45,212,160,0.3)', border: canSubmit ? 'none' : '1px solid rgba(45,212,160,0.12)', borderRadius: '14px', fontSize: '15px', fontWeight: 700, fontFamily: 'Plus Jakarta Sans, sans-serif', cursor: canSubmit ? 'pointer' : 'not-allowed', transition: 'all 0.3s', boxShadow: canSubmit ? '0 4px 24px rgba(45,212,160,0.35), inset 0 1px 0 rgba(255,255,255,0.15)' : 'none', letterSpacing: '0.02em', marginTop: '4px' }}
                onMouseEnter={e => { if (canSubmit) { (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 8px 32px rgba(45,212,160,0.5), inset 0 1px 0 rgba(255,255,255,0.15)'; (e.currentTarget as HTMLButtonElement).style.transform = 'translateY(-1px)' } }}
                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.boxShadow = canSubmit ? '0 4px 24px rgba(45,212,160,0.35), inset 0 1px 0 rgba(255,255,255,0.15)' : 'none'; (e.currentTarget as HTMLButtonElement).style.transform = 'none' }}>
                {loading ? (
        ...
        {/* Footer note */}
        <div style={{ textAlign: 'center', marginTop: '20px', fontSize: '11px', color: 'rgba(255,255,255,0.18)', letterSpacing: '0.05em' }}>
          Powered by ToleeSim · eSIM Roaming
        </div>
      </div>
    </div>
  )
...
function LogoLinkSaver({ logoShareUrl, setLogoShareUrl, setLogoUrl, userId, t, isDark }: {
  ...
  const [draft, setDraft] = useState(logoShareUrl)
...
function LPAInputView({ t, isDark, themeMode, setThemeMode, onLogout, username, userId, isAdmin = false, onSwitchToMembers }: {
  ...
  const [logoUrl, setLogoUrl] = useState<string | null>(() => localStorage.getItem('esim_logo') || null)
      ...
      <div style={{ width: '100%', maxWidth: '460px', position: 'relative', zIndex: 1 }}>
        {!isResult ? (
          ...
          <>
            ...
            <ESIMCard key={activeIndex} esim={esims[activeIndex]} logoUrl={logoUrl} index={activeIndex} total={esims.length} t={t} themeMode={themeMode} customDomain={customDomain} userId={userId} logoShareUrl={logoShareUrl || undefined} pkgData={pkgData} pkgDuration={pkgDuration} pkgRegion={pkgRegion} />
            {esims.length > 1 && (
...
export default function App() {
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('logo_url').filter('id::text', 'like', `${uid}%`).limit(1).then(({ data }) => {
