...
function LogoLinkSaver({ logoShareUrl, setLogoShareUrl, setLogoUrl, userId, t, isDark }: {
  ...
  const isDirty = draft !== logoShareUrl
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '14px 16px', background: isDark ? 'rgba(45,212,160,0.04)' : 'rgba(45,212,160,0.06)', border: `1.5px solid ${logoShareUrl ? t.teal + '44' : t.border}`, borderRadius: '12px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <label style={{ fontSize: '11px', fontWeight: 700, color: t.teal, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Link Logo Toko</label>
        {logoShareUrl && !isDirty && <span style={{ fontSize: '10px', color: t.teal, fontWeight: 600 }}>✓ Tersimpan</span>}
      </div>
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
        <input value={draft} onChange={(e) => setDraft(e.target.value)}
          placeholder="https://i.imgur.com/logomu.png"
          style={{ flex: 1, background: isDark ? 'rgba(0,0,0,0.35)' : '#fff', border: `1.5px solid ${isDirty ? t.teal : t.border}`, borderRadius: '10px', padding: '10px 14px', color: t.text, fontFamily: 'JetBrains Mono, monospace', fontSize: '11.5px', outline: 'none', transition: 'border-color 0.2s' }}
          onFocus={(e) => { e.currentTarget.style.borderColor = t.teal }}
          onBlur={(e) => { e.currentTarget.style.borderColor = isDirty ? t.teal : t.border }}
          onKeyDown={(e) => { if (e.key === 'Enter') handleSave() }}
        />
        <button onClick={handleSave} disabled={!draft || (!isDirty && !!logoShareUrl)}
          style={{ flexShrink: 0, padding: '10px 18px', borderRadius: '10px', border: 'none', background: saved ? '#16a34a' : (!draft || (!isDirty && !!logoShareUrl)) ? t.tealDim : t.teal, color: '#fff', fontSize: '12px', fontWeight: 700, fontFamily: 'Plus Jakarta Sans, sans-serif', cursor: draft ? 'pointer' : 'not-allowed', transition: 'all 0.2s', opacity: (!draft || (!isDirty && !!logoShareUrl)) && !saved ? 0.5 : 1 }}>
          {saved ? '✓ Disimpan' : saving ? '...' : 'Simpan'}
        </button>
      </div>
      <span style={{ fontSize: '10.5px', color: t.dim }}>💡 Upload ke <strong style={{ color: t.muted }}>imgur.com</strong> atau <strong style={{ color: t.muted }}>imgbb.com</strong> → copy "Direct link" → paste di sini → klik <strong style={{ color: t.teal }}>Simpan</strong></span>
    </div>
  )
}
...
function LPAInputView({ t, isDark, themeMode, setThemeMode, onLogout, username, userId, isAdmin = false, onSwitchToMembers }: {
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('logo_url').eq('id', userId).single().then(({ data }) => {
      if (data?.logo_url) { setLogoShareUrl(data.logo_url); setLogoUrl(data.logo_url); localStorage.setItem('esim_logo_url', data.logo_url) }
    })
  }, [userId])
